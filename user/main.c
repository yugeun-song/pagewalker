...
int main(int argc, char *argv[]) {
    ...
    /* Step 2/3: PUD */
    print_step(buf, &offset, (res->levels == 5) ? "Step 3: PUD (Page Upper Directory)" : "Step 2: PUD (Page Upper Directory)", 
               res->pud_base_phys, res->pud_idx, res->pud_val, (res->pud_val & 1));

    /* Step 3/4: PMD */
    print_step(buf, &offset, (res->levels == 5) ? "Step 4: PMD (Page Middle Directory)" : "Step 3: PMD (Page Middle Directory)", 
               res->pmd_base_phys, res->pmd_idx, res->pmd_val, (res->pmd_val & 1));

    /* Step 4/5: PTE */
    print_step(buf, &offset, (res->levels == 5) ? "Step 5: PTE (Page Table Entry)" : "Step 4: PTE (Page Table Entry)", 
